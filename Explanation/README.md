...
- [솔로 모방학습 실행 절차](./Imitation_Learning_Runbook.md): 현재 코드에서 실행할 정확한 생성·학습·검증·삭제 명령과 실패 대응
- [모방학습 모델 평가와 승격](./Imitation_Model_Evaluation_and_Promotion.md): offline·closed-loop 평가의 차이, 실제 `model.pt` 평가 구조와 실사용 체크포인트 생성 절차
- [모방학습 규모 이론 예측](./Imitation_Learning_Scale_Estimate.md): 실행값을 정할 때 사용한 표본 규모와 분포 이탈 근거
