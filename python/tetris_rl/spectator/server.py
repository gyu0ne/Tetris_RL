...
def main() -> None:
    parser = argparse.ArgumentParser(description="Watch a solo checkpoint choose placements")
    parser.add_argument(
        "--checkpoint",
        type=Path,
        default=Path("checkpoints/solo-imitation-versus-bootstrap-v1/model.pt"),
    )
    parser.add_argument("--seed", type=int, default=1)
    parser.add_argument("--threads", type=int, default=2)
    parser.add_argument("--bind", default="0.0.0.0:8788")
    parser.add_argument("--allow-observed", action="store_true")
    args = parser.parse_args()
    if args.seed < 0 or args.threads <= 0:
        raise ValueError("seed must be nonnegative and threads must be positive")
    ...
    print(f"Solo model spectator: http://127.0.0.1:{port_text}", flush=True)
    server.serve_forever()


def handler(controller: SpectatorController) -> type[BaseHTTPRequestHandler]:
    class SpectatorHandler(BaseHTTPRequestHandler):
        ...
        def do_POST(self) -> None:
            ...
            try:
                ...
                if path == "/api/step":
                    state = controller.step(int(payload.get("count", 1)))
                elif path == "/api/reset":
        ...
        def log_message(self, message: str, *args: object) -> None:
            print(f"spectator: {message % args}", flush=True)
