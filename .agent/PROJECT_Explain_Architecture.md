...
The implemented `crates/engine-core` contains row bitboards, piece geometry, exact observed MINSTD/`ZLOSIJT` streams, configurable spawn/hold/queue, SRS+/180 rotations, geometric reachability, and a fixed-point frame timing kernel with observed fractional spawn/kick phases. `FrameSession` preserves frame-aligned action order through hold, movement/rotation, gravity, lock/line clear, Clutch displacement and next spawn. It emits score-free clear/spin/perfect-clear and typed top-out facts. `crates/versus` owns ordered attacks, garbage generation/cancellation/transit/cap/insertion, margin scaling and the deterministic simultaneous two-player `BattleSession`. `crates/rules-tetrio` owns versioned literals and evidence. `crates/replay-conformance` owns canonical solo/battle snapshots, normalized JSON v1 integrity validation, local-trace binding, first divergence and corpus-level claim gates. `crates/manual-playground` serves a localhost-only diagnostic page and forwards ordered browser key edges to the same Rust `FrameSession`; HTML/JS never computes mechanics. `crates/arena` enumerates hold-aware afterstates, extracts integer features, generates deterministic heuristic labels and batches model-driven solo state transitions. `crates/py-bridge` transfers compact feature/teacher-score buffers and chosen indices without copying authoritative mechanics into Python. `python/tetris_rl` validates temporary shards, trains independent shared afterstate scorers, selects actual PyTorch candidates in closed loop, aggregates teacher labels on learner-visited states and promotes only a provenance-matched passing checkpoint. Raw replay/browser input is a validation adapter; the primary learner selects engine-enumerated reachable locked afterstates and does not depend on OS-event subframe timing. Details include `PROJECT_Explain_Battle_Scheduler_and_Client_Mechanics.md`, `PROJECT_Explain_Normalized_Reference_Trace.md`, `PROJECT_Explain_Manual_Playground.md`, `PROJECT_Explain_Placement_Level_Imitation_Model.md` and `PROJECT_Explain_Imitation_Evaluation.md`.
